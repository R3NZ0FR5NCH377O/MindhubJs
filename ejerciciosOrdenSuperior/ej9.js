function cervezasDentroDelNivelAlcoholico(cervezas, nivelAlcohol) {
    const cervezasFiltradas = cervezas.filter((cerveza) => cerveza.alcohol <= nivelAlcohol);
  
    const cervezasFiltradasConPropiedades = cervezasFiltradas.map((cerveza) => {
      return {
        nombre: cerveza.nombre,
        alcohol: cerveza.alcohol,
        amargura: cerveza.amargura,
      };
    });
  
    return cervezasFiltradasConPropiedades;
  }
  ...
  const nivelAlcohol = 7.0; 
  
  const cervezasFiltradas = cervezasDentroDelNivelAlcoholico(cervezas, nivelAlcohol);
